...
/**
 * @brief Encode a number into the Length Field.
 * 
 * @param buff*
 * @param len length number
 * @param n_bytes size of Length Field 
 */
inline void encode_length(octet *buff, size_t len, size_t n_bytes)
{
    ...
    if (n_bytes < 8){
        ...
        if(upper != 0 and upper != 1){
            throw invalid_length("length too lartge for length field");
        }
    }
...
/**
 * @brief Decode the Length Field into a number.
 * 
 * @param buff 
 * @param n_bytes size of Length Field 
...
inline size_t decode_length(octet *buff, size_t n_bytes)
{
    ...
    // little-endia coverted to host endian
    return le64toh(tmp);
}
